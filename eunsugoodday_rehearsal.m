...
function isCentered = move_ring_centered(drone, cam, ringCenter, ringRad)
    ...
    if ~isCentered
       ...
       if abs(distance(1)) < 0.2 && abs(distance(2) + 0.3) < 0.2
           ...
           is_rectangle = find_rectangle_in_ring(cam, ringCenter, ringRad, 'red');
        ...
        is_rectangle = find_rectangle_in_ring(cam, holeCenter, holeRad, 'red');
...
%% 링 내에 존재하는 사각형 찾기
function is_rectangle = find_rectangle_in_ring(cam, ringCenter, ringRad, targetColor)
...
%%
function [rectangleCenter, rectangleHeight, is_rectangle] = find_rectangle(cam, targetColor)
    image = snapshot(cam);
...
function movetoforward(drone, cam)
   ...
   if abs(actual_distance - 0.5) >= 0.2
       
       distance = actual_distance - 0.5;
